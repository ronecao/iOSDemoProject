...
- (void)viewDidLoad {
  ...
  NSLog(@"%@", [self formatAmount:@"1"]);//0.01
   NSLog(@"%@",  [self formatAmount:@"22"]);//0.22
    NSLog(@"%@", [self formatAmount:@"333"]);//3.33
   NSLog(@"%@",  [self formatAmount:@"4444"]);//44.44
    NSLog(@"%@",    [self formatAmount:@"10"]);//0.10
 NSLog(@"%@",    [self formatAmount:@"010"]);//0.10
 NSLog(@"%@",    [self formatAmount:@"0010"]);//0.10
 NSLog(@"%@",    [self formatAmount:@"120"]);//1.20
 NSLog(@"%@",    [self formatAmount:@"0120"]);//1.20
  NSLog(@"%@",   [self formatAmount:@"1120"]);//11.20
 NSLog(@"%@",    [self formatAmount:@"100"]);//1.00
  NSLog(@"%@",   [self formatAmount:@"101"]);//1.01
  NSLog(@"%@",   [self formatAmount:@"1000"]);//10.00
  NSLog(@"%@",   [self formatAmount:@"1010"]);//10.10
}
    ...
    for (NSDictionary *res in recordsArray)
    {
        ...
        UIAlertAction* okAction = [UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault
                                                         handler:^(UIAlertAction * action) {
                                                             ...
                                                             self->messageLabel.text=@"Processing Return Capture";
                                                             if(error) {
            ...
            UIAlertAction* okAction = [UIAlertAction actionWithTitle:[token objectForKey:@"Token"] style:UIAlertActionStyleDefault
                                                             handler:^(UIAlertAction * action) {
                                                                 ...
                                                                 if ([[token objectForKey:@"Production"] isEqualToString:@"True"]) {
                                                                     [self -> demoProductionSwitch setOn:YES];
                                                                 }
